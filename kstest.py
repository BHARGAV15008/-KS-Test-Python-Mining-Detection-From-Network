...
class KSTest:
    def __init__(self, mining_intervals, alpha: float = 0.05, k_points: int = 50):
        ...
        # Whitelist configuration (specific services only)
        self.whitelist = {
            'ips': ['192.168.1.1', '8.8.8.8', '1.1.1.1'],
            'ports': [53, 80, 443, 22, 3389],  # DNS, HTTP, HTTPS, SSH, RDP
            'protocols': ['dns', 'http', 'https', 'ssh', 'rdp']  # Removed 'tcp', 'udp'
        }
    ...
    def test_traffic(self, traffic_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for conn_id, conn_data in connections.items():
            ...
            if len(intervals) < 10 or self.is_whitelisted(conn_data):
                continue
            total_conns += 1
            ...
            # Relaxed criteria from CryptoMiningDetector
            if is_perfect_match:
                is_suspicious = True
            else:
            ...
            logger.debug(f"Connection {conn_id}: ks_stat={conn_ks_stat:.4f}, threshold={conn_threshold:.4f}, verdict={conn_details[conn_id]['verdict']}")

        # Verdict logic from CryptoMiningDetector
        ...
        # Standard result compatible with the system
        standard_result = {
            'timestamp': datetime.now().isoformat(),
